...
router.get("/questionInQuize/:id", async (req, res) => {
  ...
  if (!req.query.questionsInQuiz) {
    filter.subDomainId = obj.subDomainId;

    for (key in baseFilter) {
      filter[key] = baseFilter[key];
    }
  }

  // manual questions
  let questionList = await interactiveObjectSchema.find(filter);

  /** check if quiz uses auto generated questions */
  if (obj.isAutoGenerated && obj.criteria && !req.query.questionsInQuiz) {
    ...
    const questionsIds = questionList.map((q) => q._id.toString());

    for (const question of finalQuestions) {
      if (questionsIds.includes(question._id.toString())) continue;
